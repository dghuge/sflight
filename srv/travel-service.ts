...
export class TravelService extends cds.ApplicationService { init() {
  ...
  this.before ('NEW', Booking, async req => {
    let { maxID } = await SELECT.one (`max(BookingID) as maxID`) .from (Booking) .where (req.data) as { maxID: number }
    req.data.BookingID = ++maxID
    req.data.BookingDate = today() // REVISIT: could that be filled in by CAP automatically?
  })

  this.before ('NEW', Supplements, async req => {
    let { maxID } = await SELECT.one (`max(BookingSupplementID) as maxID`) .from (Supplements) .where (req.data) as { maxID: number }
  ...
  this.on ('UPDATE', Travel ,      (req, next) => update_totals (req, next, ['BookingFee', 'GoGreen']))
  this.on ('UPDATE', Booking ,     (req, next) => update_totals (req, next, ['FlightPrice']))
  this.on ('UPDATE', Supplements , (req, next) => update_totals (req, next, ['Price']))
  this.on ('DELETE', Booking ,     (req, next) => update_totals (req, next))
  this.on ('DELETE', Supplements , (req, next) => update_totals (req, next))
  ...
  async function update_totals (req: cds.Request, next: Function, fields?: string[]) {
    ...
    await cds.run(`UPDATE ${Travel } SET TotalPrice = coalesce (BookingFee,0)
     + coalesce(GreenFee,0)
     + ( SELECT coalesce (sum(FlightPrice),0) from ${Booking } where to_Travel_TravelUUID = TravelUUID )
     + ( SELECT coalesce (sum(Price),0) from ${Supplements } where to_Travel_TravelUUID = TravelUUID )
  ...
  async function update_totalsGreen(TravelUUID: string) {
    const { GoGreen } = await SELECT.one .from(Travel ) .columns('GoGreen') .where({ TravelUUID })
    if (GoGreen) {
      await UPDATE(Travel , TravelUUID)
